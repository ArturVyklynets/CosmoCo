...
if ($session_id === null || $session_id <= 0) {
    http_response_code(400);
    echo json_encode(['error' => 'Відсутній або некоректний session_id']);
    exit;
}
...
if ($total > 1000) {
    http_response_code(400);
    echo json_encode(['error' => 'Сума повинна бути менше за 1000']);
    exit;
}

$stmt = $conn->prepare("INSERT INTO games (player_id, session_id, kronus, lyrion, mystara, eclipsia, fiora) VALUES (?, ?, ?, ?, ?, ?, ?)");
$stmt->bind_param("iiiiiii", $player_id, $session_id, $kronus, $lyrion, $mystara, $eclipsia, $fiora);
